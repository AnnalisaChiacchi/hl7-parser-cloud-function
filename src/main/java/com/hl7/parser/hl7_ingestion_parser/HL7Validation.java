	...
	public JsonObject validator(JsonObject obj) {
		...
		if ( ! obj.isJsonNull()) {
			...
			try {
				...
				//String orc2 = terser.get("/.ORC-2-1");	
				...
				if ( type.equals("ORU")) {
					...
					/*
					if (msh != null) {
						objMsh.addProperty("hl7messagetype", msh.getMsh9_MessageType().getCm_msg1_MessageType().getValue() + "_" + msh.getMsh9_MessageType().getCm_msg2_TriggerEvent().getValue());
